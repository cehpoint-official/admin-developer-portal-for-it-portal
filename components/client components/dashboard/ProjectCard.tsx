    ...
    <Link href={`/client/projects/${project.id}`}>
      <Card className="h-full overflow-hidden transition-all hover:shadow-md hover:border-primary/20">
        ...
        <CardContent className="p-4 pt-2">
          <p className="text-muted-foreground text-sm line-clamp-2 mb-4">{project.projectOverview}</p>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
              <div className="flex items-center gap-1">
                ...
                <span>{formatDistanceToNow(project.submittedAt, { addSuffix: true })}</span>
              </div>
{/* 
              {project.teamMembers && (
                <div className="flex items-center gap-1">
        ...
        <CardFooter className="p-4 pt-0 flex justify-between items-center">
          <div className="text-xs text-muted-foreground">Budget: ₹{project.projectBudget.toLocaleString()}</div>
